...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if(r.getRightX() < getLeftX()||r.getLeftX() > getRightX()){
        return false;
    }
    // 2. when one is above the other
    else if(getTopY() > r.getBottomY() || getBottomY() < r.getTopY()){
        return false;
    }
    else{
        return true;
...
void Rect::draw() const {
    ...
    // Upper left corner
    glVertex2i((center.x-(getWidth()/2)), (center.y+(getHeight()/2)));
    // Bottom left corner
    glVertex2i((center.x-(getWidth()/2)), (center.y-(getHeight()/2)));
    // Upper right corner
    glVertex2i((center.x+(getWidth()/2)), (center.y+(getHeight()/2)));
